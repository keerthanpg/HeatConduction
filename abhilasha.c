...
{     int i,j,n;
           ...
           {  
			   for( n=0;n<3;n++ )
                  {
					   x[i][n] += v[i][n]*timestep+0.5*(a[i][0][n])*timestep*timestep;
                    }
 ...
 void velocity(double v[no_moles][3], double a[no_moles][3][2] , double *K)
{
	 int i,n;
     *K=0.0;
      ...
      {    
		  for(n=0;n<3;n++)
		 {
			   v[i][n] += 0.5 * ( a[i][n][0] + a[i][n][1] ) * timestep;
			   *K+= 0.5*m*v[i][n]*v[i][n];
          }  
       }
       return;
...
void acceleration( double x[no_moles][3] ,double a[no_moles][3][2] ,double *U)
   {  double r;
      double x_diff[3];
      int i,j,n,mo;
      *U=0.0 ;
                        ...
                        if  (r<2.5)
 			            {  
						   *U+=  4.0*epsilon*( pow((sigma/r),12.0)- pow((sigma/r),6.0)); 
 			                for (mo=0; mo<3; mo++)
						   {
						    a[i][mo][1] -= 4.0*epsilon*(x_diff[0]/r)*( 6.0*pow((sigma/r),7)-12.0* pow((sigma/r),13));
						    a[j][mo][1]  += 4.0*epsilon*(x_diff[0]/r)*( 6.0*pow((sigma/r),7)-12.0* pow((sigma/r),13));
						   }
