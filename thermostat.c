...
double pi=3.14;
int no_moles=1000; 
double L=1000.000;//Number density = 0.728
...
{
	x[i]=L/(double)no_moles*(double)i;
	fprintf(pos, "\n%d %lf ", i, x[i]);
...
FILE* Inivel=fopen("RandVel.txt", "r");
for(i=0;i<no_moles;i++)
...
void force(double x[no_moles], FILE* acc, double a[no_moles][2], double *U, double time)
{
	int i, k, j;
	double x_diff, dis;
    ...
    {         
		  ...
		  if(i+1<no_moles)
		{
			...
			if(dis<dis_far_nbour)              
			{
				*U += 4.0*epsilon*(pow((sigma/dis),12)-pow((sigma/dis),6));
				...
				a[i][1] += 4.0*epsilon/m*(-12*pow(sigma/dis, 14)+6*pow(sigma/dis, 8))*x_diff;
		...
		for(j=i+1;j<no_moles;j++)
        {
			...
			if(dis<dis_far_nbour)              
			{
				*U += 4.0*epsilon*(pow((sigma/dis),12)-pow((sigma/dis),6));									
				a[i][1] -= 4.0*epsilon/m*(-12*pow(sigma/dis, 14)+6*pow(sigma/dis, 8))*x_diff;
				a[j][1] += 4.0*epsilon/m*(-12*pow(sigma/dis, 14)+6*pow(sigma/dis, 8))*x_diff;				
			}			
